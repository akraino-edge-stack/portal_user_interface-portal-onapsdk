/* Retrieved from https://cdnjs.cloudflare.com/ajax/libs/ng-table/1.0.0/ng-table.js */
...
(function(){
     ...
     *
     * Supported events: * afterCreated - raised when a new instance of
     * `NgTableParams` has finished being constructed * afterReloadData - raised
     * when the `reload` event has finished loading new data * datasetChanged -
     * raised when `settings` receives a new data array * pagesChanged - raised
     * when a new pages array has been generated
     */
        ...
        function createEventSubscriptionFn(eventName){
            ...
            return function subscription(handler/*
                ...
                return scope.$on('ngTable:' + eventName, function(event, params/*
                                                                                 ...
                                                                                 * ...args
                                                                                 */){
                    // don't send events published by the internal NgTableParams
                    // created by ngTableController
...
(function(){
     ...
     * @description Allows for the configuration of the ngTableDefaultGetData
     *              service.
     *
     * Set filterFilterName to the name of a angular filter that knows how to
     * apply the values returned by `NgTableParams.filter()` to restrict an
     * array of data.
     *
     * Set sortingFilterName to the name of a angular filter that knows how to
     * apply the values returned by `NgTableParams.orderBy()` to sort an array
     * of data.
     *
     * Out of the box the `ngTableDefaultGetData` service will be configured to
     * use the angular `filter` and `orderBy` filters respectively
     */
    function ngTableDefaultGetDataProvider(){
         ...
         * @description A default implementation of the getData function that
         *              will apply the `filter`, `orderBy` and paging values
         *              from the `NgTableParams` instance supplied to the data
         *              array supplied.
         *
         * The outcome will be to return the resulting array and to assign the
         * total item count after filtering to the `total` of the
...
(function () {
    ...
    angular.module('ngTable').factory('ngTableColumn', [function () {
         ...
         *
         * @param {Object}
         *            column an existing $column or simple column data object
         * @param {Scope}
         *            defaultScope the $scope to supply to the $column getter
         *            methods when not supplied by caller
         * @param {Array}
         *            columns a reference to the columns array to make available
         *            on the context supplied to the $column getter methods
         * @returns {Object} a $column object
         */
        function buildColumn(column, defaultScope, columns){
            // note: we're not modifying the original column object. This helps
            // to avoid unintended side affects
            ...
            for (var prop in defaults) {
                ...
                if(!angular.isFunction(extendedCol[prop])){
                    ...
                    // - this is to ensure consistency with how ngTable.compile
                    // builds columns
                    // - note that the original column object is being
                    // "proxied"; this is important
                    // as it ensure that any changes to the original object will
                ...
                (function(prop1){
                    // satisfy the arguments expected by the function returned
                    // by parsedAttribute in the ngTable directive
...
(function(){
    ...
    angular.module('ngTable').factory('NgTableParams', ['$q', '$log', '$filter', 'ngTableDefaults', 'ngTableDefaultGetData', 'ngTableEventsChannel', function($q, $log, $filter, ngTableDefaults, ngTableDefaultGetData, ngTableEventsChannel) {
        ...
        var NgTableParams = function(baseParameters, baseSettings) {
            ...
            // the ngTableController "needs" to create a dummy/null instance and
            // it's important to know whether an instance
            // is one of these
            ...
            var self = this,
                ...
                defaultFilterOptions = {
                    filterComparator: undefined, // look for a substring
                                                    // match in case insensitive
                                                    // way
                    filterDelay: 500,
                    filterDelayThreshold: 10000, // size of dataset array
                                                    // that will trigger the
                                                    // filterDelay being applied
                    filterFilterName: undefined, // when defined overrides
                                                    // ngTableDefaultGetDataProvider.filterFilterName
                    filterFn: undefined, // when defined overrides the filter
                                            // function that
                                            // ngTableDefaultGetData uses
                    filterLayout: 'stack' // alternative: 'horizontal'
                },
                defaultGroupOptions = {
                    defaultSort: 'asc', // set to 'asc' or 'desc' to apply
                                        // sorting to groups
                    isExpanded: true
                },
             ...
             * @param {string}
             *            parseParamsFromUrl Flag if parse parameters like in
             *            url
             ...
             *
             * @param {string}
             *            newSettings New settings or undefined
             * @returns {Object} Current settings or `this`
             */
            this.settings = function(newSettings) {
                if (angular.isDefined(newSettings)) {

                    // todo: don't modify newSettings object: this introduces
             ...
             * @description If parameter page not set return current page else
             *              set current page
             *
             * @param {string}
             *            page Page number
             ...
             * @description If parameter total not set return current quantity
             *              else set quantity
             *
             * @param {string}
             ...
             * @description If parameter count not set return current count per
             *              page else set count per page
             *
             * @param {string}
             ...
             * @description If 'filter' parameter not set return current filter
             *              else set current filter
             *
             * Note: when assigning a new filter,
             * {@link NgTableParams#page page} will be set to 1
             *
             * @param {Object|Boolean}
             *            filter 'object': new filter to assign or 'true': to
             *            return the current filter minus any insignificant
             *            values (null, undefined and empty string); or
             ...
             * @description If 'group' parameter is not set, return current
             *              grouping. Otherwise set current group.
             *
             * @param {string|Function|Object}
             *            group New group field
             * @param {string}
             *            sortDirection Optional direction that the list of
             ...
             * @description If 'sorting' parameter is not set, return current
             *              sorting. Otherwise set current sorting.
             *
             * @param {string}
             *            sorting New sorting
             ...
             *            field Field name
             * @param {string}
             *            direction Optional direction of sorting ('asc' or
             *            'desc')
             ...
             * @description Return object of sorting parameters for angular
             *              filter
             *
             ...
             *
             * When no arguments supplied, the current parameter state of this
             * `NgTableParams` instance will be used
             *
             * @param {boolean}
             *            currentPage which page must be active
             * @param {boolean}
             *            totalItems Total quantity of items
             * @param {boolean}
             *            pageSize Quantity of items on page
             * @param {number}
             *            maxBlocks Quantity of blocks for pagination
             ...
             * @description Return true when a change to this `NgTableParams`
             *              instance should require the reload method to be run
             *              so as to ensure the data presented to the user
             *              reflects the `NgTableParams`
             *
             * Note that this method will return false when the reload method
             * has run but fails. In this case `hasErrorState` will return true.
             */
             ...
             * @description Determines if NgTableParams#filter has significant
             *              filter value(s) (any value except null, undefined,
             *              or empty string)
             * @returns {Boolean} true when NgTableParams#filter has at least
             ...
             *              `NgTableParams.filters`require the reload method to
             *              be run so as to ensure the data presented to the
             *              user reflects these filters
             */
             ...
             *
             * @param {boolean}
             *            asString flag indicates return array of string or
             *            object
             * @returns {Array} If asString = true will be return array of url
             *          string parameters else key-value object
             */
            ...
            this.reload = function() {
                ...
                return pData.then(function(data) {
                    ...
                    self.data = data;
                    // note: I think it makes sense to publish this event even
                    // when data === oldData
                    // subscribers can always set a filter to only receive the
                    // event when data !== oldData
             ...
             * @description Return true when an attempt to `reload` the current
             *              `parameter` values have resulted in a failure
             *
             * This method will continue to return true until the reload is
             * successfully called or when the `parameter` values have changed
             */
            ...
            function optimizeFilterDelay(){
                // don't debounce by default filter input when working with
                // small synchronous datasets
            ...
            function getDefaultSettingFns(){
                 ...
                 *
                 * Called by `NgTableParams` whenever it considers new data is
                 * to be loaded
                 *
                 * @param {Object}
                 *            params the `NgTableParams` requesting data
                 */
                 ...
                 *
                 * Called by `NgTableParams` whenever it considers new data is
                 * to be loaded and when a `group` value has been assigned
                 *
                 * @param {Object}
                 *            params the `NgTableParams` requesting data
                 */
                ...
                function getPath (obj, ks) {
                    ...
                    // If we still have elements in the path array and the
                    // current
            ...
            var settings = {
                $loading: false,
                dataset: null, // allows data to be set when table is
                                // initialized
                total: 0,
                defaultSort: 'desc',
            ...
            // run events during construction after the initial create event.
            // That way a consumer
            // can subscribe to all events for a table without "dropping" an
            // event
...
(function(){
     ...
     *
     * @description Each {@link ngTable ngTable} directive creates an instance
     *              of `ngTableController`
     */
    angular.module('ngTable').controller('ngTableController', ['$scope', 'NgTableParams', '$timeout', '$parse', '$compile', '$attrs', '$element',
        ...
        function($scope, NgTableParams, $timeout, $parse, $compile, $attrs, $element, ngTableColumn, ngTableEventsChannel) {
            ...
            // until such times as the directive uses an isolated scope, we need
            // to ensure that the check for
            // the params field only consults the "own properties" of the
            // $scope. This is to avoid seeing the params
            ...
            // CRITICAL: the watch must be for reference and NOT value equality;
            // this is because NgTableParams maintains
            // the current data page as a field. Checking this for value
            // equality would be terrible for performance
            // and potentially cause an error if the items in that array has
            // circular references
            ...
            this.compileDirectiveTemplates = function () {
                if (!$element.hasClass('ng-table')) {
                    ...
                    // $element.find('> thead').length === 0 doesn't work on
                    // jqlite
            ...
            this.loadFilterData = function ($columns) {
                angular.forEach($columns, function ($column) {
                    ...
                    // if we're working with a deferred object or a promise,
                    // let's wait for the promise
                    ...
                    if ((angular.isObject(result) && (angular.isObject(result.promise) || angular.isFunction(result.then)))) {
                        ...
                        delete $column.filterData;
                        return pData.then(function(data) {
                            // our deferred can eventually return arrays,
                            // functions and objects
                            if (!angular.isArray(data) && !angular.isFunction(data) && !angular.isObject(data)) {
                                // if none of the above was found - we just want
                                // an empty array
                                data = [];
                            }
                            $column.data = data;
                    ...
                    // otherwise, we just return what the user gave us. It could
                    // be a function, array, object, whatever
                    else {
            ...
            this.setupBindingsToInternalScope = function(tableParamsExpr){
                ...
                // note: this we're setting up watches to simulate angular's
                // isolated scope bindings

                // note: is REALLY important to watch for a change to the
                // ngTableParams *reference* rather than
                // $watch for value equivalence. This is because ngTableParams
                // references the current page of data as
            ...
            function setupGroupRowBindingsToInternalScope(){
                ...
                if ($attrs.showGroup) {
                    ...
                    if (showGroupGetter.assign){
                        // setup two-way databinding thus allowing
                        // ngTableGrowRow to assign to the showGroup expression
            ...
            return {
                ...
                compile: function(element) {
                    ...
                    angular.forEach(dataRow.find('td'), function(item) {
                        ...
                        var parsedAttribute = function(attr) {
                            ...
                            getter.assign = function($scope, value){
                                ...
                                if (parsedExpr.assign) {
                                    // we should be writing back to the parent
                                    // scope as this is where the expression
                                    // came from
                        ...
                        if (titleExpr){
                            el.attr('data-title-text', '{{' + titleExpr + '}}'); // this
                                                                                    // used
                                                                                    // in
                                                                                    // responsive
                                                                                    // table
                        }
                        // NOTE TO MAINTAINERS: if you add extra fields to a
                        // $column be sure to extend ngTableColumn with
                        ...
                        if (groupRow || el.attr("ng-if")){
                            // change ng-if to bind to our column definition
                            // which we know will be writable
                            // because this will potentially increase the $watch
                            // count, only do so if we already have an
                            // ng-if or when we definitely need to change
                            // visibility of the columns.
                            // currently only ngTableGroupRow directive needs to
                            // change visibility
...
(function(){
     ...
     * @restrict A
     *
     * @description A dynamic version of the {@link ngTable ngTable} directive
     *              that accepts a dynamic list of columns definitions to render
     */
...
(function(){
     ...
     *
     * This allows the $columns array to be accessed outside of the html table
     * markup
     */
...
(function(){
     ...
     * @restrict A
     *
     * @description Takes the array returned by $column.filterData and makes it
     *              available as `$selectData` on the `$scope`.
     *
    ...
    function ngTableSelectFilterDs(){
        ...
        // this is to allow this directive to be combined with other directives
        // that do
