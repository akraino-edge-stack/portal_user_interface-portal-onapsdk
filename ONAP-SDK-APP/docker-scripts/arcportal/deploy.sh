...
docker run --detach --name $CONTAINER_NAME -p:$HOST_PORT:8080 -e DB_IP_PORT="$DB_IP_PORT" -e MARIADB_AKRAINO_PASSWORD="$MARIADB_AKRAINO_PASSWORD" -e ARC_URL="$ARC_URL" -e ARC_PROXY="$ARC_PROXY" -e ARC_USER="$ARC_USER" -e ARC_PASSWORD="$ARC_PASSWORD" $IMAGE
sleep 10
